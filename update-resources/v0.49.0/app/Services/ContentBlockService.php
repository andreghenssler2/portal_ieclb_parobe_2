...
final class ContentBlockService
{
    ...
    private static function renderVideo(array $data): string
    {
        ...
        $html = '<figure class="portal-video-block my-4">';

        if ($embed !== '') {
            $html .= '<div class="ratio ratio-16x9 rounded overflow-hidden bg-dark">'
                . '<iframe src="'
                . self::escape($embed)
                . '" title="Vídeo"'
                . ' loading="lazy"'
            ...
            $html .= '<div class="border rounded p-3 bg-body-tertiary">'
                . '<div class="small text-secondary mb-2">'
                . 'Este endereço não possui formato de incorporação reconhecido.'
                . '</div>'
    ...
    private static function videoUrl(string $value): string
    {
        $value = trim(
            html_entity_decode(
                $value,
        ...
        // Links copiados sem protocolo, por exemplo:
        // youtube.com/watch?v=..., youtu.be/... ou vimeo.com/...
        if (preg_match(
    ...
    private static function videoEmbedUrl(string $url): string
    {
        ...
        if ($host === 'youtu.be') {
            ...
            return self::youtubeEmbedUrl(
                $id,
                self::youtubeStartSeconds($query)
            );
        }
        ...
        if (in_array(
            ...
            if (
                ...
                $rawId = (string)($segments[1] ?? '');
            } else {
                $rawId = (string)($segments[0] ?? '');
            }
    ...
    private static function youtubeEmbedUrl(
        string $id,
        int $startSeconds = 0
    ): string {
    ...
    /**
     * Aceita t=90, t=90s, t=1m30s, t=1h2m3s e start=90.
     *
     * @param array<string,mixed> $query
     */
    private static function youtubeStartSeconds(array $query): int
    {
        $raw = trim(
            (string)($query['start'] ?? $query['t'] ?? '')
        );
