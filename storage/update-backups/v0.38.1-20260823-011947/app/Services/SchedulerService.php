...
final class SchedulerService
{
    ...
    public static function registry(): array
    {
        return [
            ...
            'otimizar_midias_pendentes' => [
                'name' => 'Otimizar imagens pendentes',
                'description' => 'Processa até 3 imagens sem variantes por execução. Vem desativada para evitar carga inesperada no servidor.',
                'interval' => 60,
                'enabled' => false,
                'priority' => 50,
            ],
    ...
    private static function dispatch(PDO $pdo, string $slug): array
    {
        return match ($slug) {
            ...
            'otimizar_midias_pendentes' => self::optimizePendingMedia($pdo),
    ...
    /** @return array{status:string,message:string} */
    private static function optimizePendingMedia(PDO $pdo): array
    {
        if (!class_exists('ImageOptimizationService')) {
            return ['status' => 'ignorado', 'message' => 'ImageOptimizationService não está disponível.'];
        }
        if (!self::tableExists($pdo, 'midias') || !self::tableExists($pdo, 'midia_variantes')) {
            return ['status' => 'ignorado', 'message' => 'Estrutura de variantes de imagem não está instalada.'];
        }
        if (ImageOptimizationService::driver() === 'none') {
            return ['status' => 'ignorado', 'message' => 'GD ou Imagick não está disponível no servidor.'];
        }
        ...
        $stmt = $pdo->query(
            "SELECT m.id
             ...
             WHERE m.mime_type IN ('image/jpeg','image/png','image/webp')
               AND NOT EXISTS (
                   SELECT 1 FROM midia_variantes v WHERE v.midia_id=m.id
               )
             ORDER BY m.id ASC
             LIMIT 3"
        );
        ...
        if (!$ids) {
            return ['status' => 'ok', 'message' => 'Nenhuma imagem pendente de otimização.'];
        }
        ...
        return [
            'status' => $failed > 0 && $processed === 0 ? 'ignorado' : 'ok',
            'message' => "Imagens processadas: {$processed}; falhas ignoradas: {$failed}.",
        ];
    }
