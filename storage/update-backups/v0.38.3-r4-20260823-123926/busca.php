...
<section class="container py-5">
    <div class="search-page mx-auto">
        ...
        <form class="input-group input-group-lg mb-4" method="get" action="<?= e(url('busca')) ?>"><input
                class="form-control" type="search" name="q" value="<?= e($q) ?>"
                placeholder="Buscar notícias, páginas, eventos, documentos, lideranças..." minlength="2"
                required><button class="btn btn-primary">Buscar</button></form>
        <?php if ($q !== '' && mb_strlen($q) < 2): ?>
            <div class="alert alert-warning">Digite pelo menos 2 caracteres.</div><?php elseif ($q !== ''): ?>
            <p class="text-secondary mb-4"><?= count($results) ?> resultado(s) para <strong><?= e($q) ?></strong>.</p>
            <?php if (!$results): ?>
                <div class="alert alert-light border">Nenhum conteúdo encontrado.</div><?php endif; ?>
            <div class="list-group list-group-flush search-results"><?php foreach ($results as $r): ?><a
                        class="list-group-item list-group-item-action px-0 py-3"
                        href="<?= e(contentUrl((string) $r['type'], (string) $r['slug'])) ?>">
                        <div class="d-flex justify-content-between gap-3">
                            <div><span class="badge text-bg-light border mb-2"><?= e($labels[$r['type']] ?? $r['type']) ?></span>
                                <h2 class="h5 mb-1"><?= e($r['titulo']) ?></h2>
                                <p class="text-secondary mb-0"><?= e($r['resumo'] ?: portalExcerpt($r['conteudo'], 180)) ?></p>
                            </div><?php if ($r['dt']): ?><small
                                    class="text-secondary text-nowrap"><?= e(formatDateOnlyBr($r['dt'])) ?></small><?php endif; ?>
                        </div>
